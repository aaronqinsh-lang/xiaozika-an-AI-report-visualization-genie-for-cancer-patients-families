...
import { 
  Edit3, Save, FileText, Stethoscope, BriefcaseMedical, 
...
export const ArchiveView: React.FC<ArchiveProps> = ({ 
  ...
  const handleEmergencyChange = (field: string, value: string) => {
    onUpdateProfile({
      ...profile,
      emergency: {
        ...(profile.emergency || { name: '未设置', relation: '-', phone: '-' }),
        [field]: value
      }
    });
  };
  ...
  return (
    <div className={`space-y-8 ${seniorMode ? 'pb-48' : 'pb-32'} animate-in slide-in-from-right-10 duration-500`}>
      ...
      <div className="flex justify-between items-center px-1">
        <h1 className={`${seniorMode ? 'text-4xl' : 'text-3xl'} font-black text-gray-900 tracking-tight`}>健康档案</h1>
        <button 
        ...
        >
          {isEditing ? <><Save size={18} /> 保存更改</> : <><Edit3 size={18} /> 编辑档案</>}
        </button>
      </div>
      ...
      <div className={`bg-white ${seniorMode ? 'p-9' : 'p-7'} rounded-[3rem] shadow-sm border border-purple-50 space-y-6`}>
        <div className="flex items-center gap-6">
          <div className={`${seniorMode ? 'w-28 h-28 text-4xl' : 'w-24 h-24 text-3xl'} bg-gradient-to-tr from-purple-100 to-purple-50 rounded-[2.5rem] flex items-center justify-center text-purple-600 font-black shadow-inner`}>
        ...
        <div className="pt-6 border-t border-gray-50">
           <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-3">
             <FileText size={14} className="text-purple-400" /> 既往病史与备注记录
           </p>
           {isEditing ? (
             <textarea 
             ...
             <div className={`${seniorMode ? 'text-lg' : 'text-sm'} bg-gray-50/50 p-7 rounded-[2.5rem] leading-relaxed text-gray-600 font-medium`}>
               {profile.medical_history || '点击编辑录入您的既往病史，AI 助手将结合此信息为您提供更精准的指标分析。'}
             </div>
           )}
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-end px-2">
           <h3 className={`${seniorMode ? 'text-2xl' : 'text-lg'} font-black text-gray-800 flex items-center gap-2`}><Activity size={20} className="text-purple-500" /> 检查记录列表</h3>
        ...
        <div className="space-y-3">
          {records.length > 0 ? records.slice(0, 5).map(r => {
            ...
            return (
              <div key={r.id} className="bg-white p-5 rounded-[2.5rem] border border-gray-50 flex items-center gap-4 group hover:border-purple-200 transition-all shadow-sm active:scale-98">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {hasTumor && <span className="text-[8px] bg-purple-100 text-purple-600 px-2 py-0.5 rounded-full font-black">重点关注</span>}
                  </div>
            ...
            <div className="bg-white/50 border-2 border-dashed border-gray-100 p-16 rounded-[3rem] text-center">
               <p className="text-sm text-gray-300 font-bold">暂无上传记录。请通过 AI 助手上传首份化验单。</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center px-2">
          ...
          {isEditing && (
            <button onClick={handleAddDoctor} className="text-blue-500 flex items-center gap-1.5 text-xs font-black uppercase tracking-wider bg-blue-50 px-4 py-2 rounded-full active:scale-95 transition-all">
          ...
          {profile.doctors?.length ? profile.doctors.map(d => (
            <div key={d.id} className="bg-white p-6 rounded-[2.8rem] border border-gray-50 shadow-sm flex items-center justify-between group transition-all hover:border-purple-200">
             ...
             <div className="bg-gray-50/50 p-8 rounded-[2.5rem] text-center border-2 border-dashed border-gray-100">
               <p className="text-xs text-gray-300 font-bold">点击“编辑档案”添加您的主治医师。</p>
             </div>
          )}
        </div>
      </div>
            ...
            {isEditing ? (
              ...
              <>
                <p className={`${seniorMode ? 'text-2xl' : 'text-lg'} font-black text-gray-900`}>
                  {profile.emergency?.name || '未设置'} · <span className="text-red-600 opacity-80 font-bold uppercase tracking-widest text-xs">{profile.emergency?.relation || '-'}</span>
                </p>
